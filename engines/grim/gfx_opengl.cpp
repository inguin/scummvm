...
void GfxOpenGL::drawEMIModelFace(const EMIModel *model, const EMIMeshFace *face) {
	...
	for (uint j = 0; j < face->_faceLength * 3; j++) {
		...
		if (!_currentShadowArray) {
			...
			byte a = (byte)(alpha * ((model->_meshAlphaMode == Actor::AlphaReplace || face->_flags & EMIMeshFace::kAlphaBlend) ? model->_colorMap[index].a * _currentActor->getLocalAlpha(index) : 255.f));
			glColor4ub(r, g, b, a);
		}
